...
<section class="section">
    ...
    <form action="{{ url('update_doctor_profile') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="card">
                ...
                <div class="form-group">
                    ...
                    <select name="language" class="form-control">
                        @foreach ($languages as $language)
                            <option value="{{ $language->name }}" {{ $doctor->language == $language->name ? 'selected' : '' }}>{{ $language->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="card-body">
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-lg-6 form-group base_on_com {{ $doctor->based_on != 'commission' ? 'hide' : '' }}">
                        ...
                        <input type="text" name="commission_amount" disabled {{ $doctor->based_on == 'commission_amount' ? 'required' : '' }} value="{{ $doctor->commission_amount }}" class="form-control base_on_com_text @error('commission_amount') is-invalid @enderror">
