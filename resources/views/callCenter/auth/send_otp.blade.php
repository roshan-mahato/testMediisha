...
@section('title',__('Verified CallCenter'))
        ...
        <div class="p-4 m-3">
          ...
          <img src="{{ $app_logo->logo }}" alt="logo" width="180" class="mb-5 mt-2">
            ...
            @if (!session('error'))
                ...
                @endif
            @endif
            <form action="{{ url('callcenter/verify_otp') }}" method="post">
                @csrf
            ...
            <div class="text-muted text-center">
                {{__("Don't Send A Code?")}}<a href="{{ url('callcenter/send_otp/'.$user->id) }}">{{__('Send Code Again')}}</a>
            </div>
